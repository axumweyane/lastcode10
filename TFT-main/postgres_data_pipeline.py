...
class PostgresDataPipeline:
    ...
    def build_dataset(
        ...
        """
        logger.info(
            f"Building dataset for {len(symbols)} symbols from {start_date} to {end_date}"
        )
        ...
        if include_earnings:
            try:
                earnings = self.loader.load_earnings_calendar(
                    symbols, start_date, end_date
                )
                if not earnings.empty:
                    # Create earnings flag for upcoming earnings
                    earnings_dates = set(
                        earnings["earnings_date"].dt.strftime("%Y-%m-%d")
                    )
                    df["earnings_flag"] = (
                        df["date"].astype(str).isin(earnings_dates).astype(int)
                    )

                    # Add days until next earnings
                    df["days_to_earnings"] = self._calculate_days_to_earnings(
                        df, earnings
                    )
                    ...
                    df["earnings_flag"] = 0
                    df["days_to_earnings"] = (
                        999  # Large number for no upcoming earnings
                    )
            except Exception as e:
    ...
    def _calculate_days_to_earnings(
        self, df: pd.DataFrame, earnings: pd.DataFrame
    ) -> pd.Series:
    ...
    def _create_target_variable(
        self, df: pd.DataFrame, target_type: str, horizon: int
    ) -> pd.DataFrame:
        """Create target variable based on specified type and horizon"""
        logger.info(
            f"Creating {target_type} target variable with {horizon}-day horizon..."
        )
    ...
    def _filter_sufficient_data(
        self, df: pd.DataFrame, min_observations: int = 100
    ) -> pd.DataFrame:
        ...
        logger.info(
            f"Filtered symbols: {original_symbols} -> {df_filtered['symbol'].nunique()}"
        )
        logger.info(
            f"Removed {original_symbols - df_filtered['symbol'].nunique()} symbols with < {min_observations} observations"
        )
    ...
    def get_feature_columns(self, df: pd.DataFrame) -> Dict[str, List[str]]:
        ...
        return {
            "static_categoricals": [
                col for col in static_categoricals if col in df.columns
            ],
            "static_reals": [col for col in static_reals if col in df.columns],
            "time_varying_known_reals": [
                col for col in time_varying_known_reals if col in df.columns
            ],
            "time_varying_unknown_reals": [
...
if __name__ == "__main__":
    ...
    try:
        ...
        if available_symbols:
            ...
            print(
                f"Time varying unknown: {len(features['time_varying_unknown_reals'])}"
            )
