...
from strategies.config import (
    MomentumConfig, StatArbConfig, EnsembleConfig, FXConfig,
...
def fx_data():
    ...
    for pair in pairs:
        ...
        for i, dt in enumerate(dates):
            rows.append({"symbol": pair, "timestamp": dt, "close": max(prices[i], 0.1), "volume": 0})
    return pd.DataFrame(rows)
...
def test_all_stock_strategies_produce_output(stock_data):
    ...
    strategies = [
        ("momentum", CrossSectionalMomentum(MomentumConfig(
    ...
    for name, strategy in strategies:
        ...
        assert isinstance(output, StrategyOutput), f"{name} failed to produce StrategyOutput"
...
def test_fx_strategies_produce_output(fx_data):
    ...
    strategies = [
        ...
        ("fx_momentum", FXMomentumStrategy(FXMomentumConfig(
            enabled=True, min_lookback_days=21, signal_threshold=0.3))),
        ("fx_vol_breakout", FXVolBreakoutStrategy(FXVolBreakoutConfig(
...
def test_ensemble_combines_all_strategies(stock_data, fx_data):
    ...
    sr = SectorRotationStrategy(SectorRotationConfig(
        enabled=True, min_tilt_threshold=0.05))
    ...
    fx_mom = FXMomentumStrategy(FXMomentumConfig(
        enabled=True, min_lookback_days=21, signal_threshold=0.3))
...
def test_strategies_handle_overlapping_symbols(stock_data):
    ...
    mom = CrossSectionalMomentum(MomentumConfig(
        enabled=True, min_history_days=250, min_avg_dollar_volume=0))
    mr = MeanReversionStrategy(MeanReversionConfig(
        enabled=True, entry_zscore=0.5, hurst_threshold=0.55))
