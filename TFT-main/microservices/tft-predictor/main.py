...
class TFTPredictor:
        ...
        try:
            ...
            try:
                latest_version = client.get_latest_versions(
                    name="tft_trading_model", stages=["Production"]
                )[0]
    ...
    def create_new_model(self) -> nn.Module:
        ...
        class SimplifiedTFT(nn.Module):
            def __init__(
                self, input_dim: int = 50, hidden_dim: int = 128, num_layers: int = 3
            ):
    ...
    async def initialize_preprocessing_artifacts(self):
        ...
        await self.redis_client.set(
            "tft_features", json.dumps(self.feature_columns), ex=3600
        )

    async def predict_single(
        self, ticker: str, features: Dict[str, float], horizons: List[int]
    ) -> Dict:
            ...
            for i, horizon in enumerate(horizons):
                if horizon in horizon_mapping:
                    ...
                    if horizon_key in predictions:
                        result["predictions"][f"{horizon}h"] = float(
                            predictions[horizon_key].cpu().item()
                        )
                        result["confidence"][f"{horizon}h"] = float(
                            confidence[0, i].cpu().item()
                        )

            # Cache prediction
            cache_key = (
                f"prediction:{ticker}:{datetime.utcnow().strftime('%Y-%m-%d-%H')}"
            )
        ...
        try:
            with mlflow.start_run():
                ...
                for epoch in range(50):  # Reduced for demo
                    ...
                    for batch_idx, (batch_x, batch_y) in enumerate(X_train):
                        ...
                        for horizon in ["1h", "4h", "24h"]:
                            if horizon in predictions and horizon in batch_y:
                                loss += criterion(
                                    predictions[horizon], batch_y[horizon]
                                )
                    ...
                    if epoch % 10 == 0:
                        logger.info(
                            f"Epoch {epoch}, Loss: {total_loss/len(X_train):.4f}"
                        )
                        mlflow.log_metric(
                            "training_loss", total_loss / len(X_train), step=epoch
                        )

                # Evaluate model
                val_mape = await self.evaluate_model(
                    model, training_data[-1000:]
                )  # Use last 1000 for validation
                ...
                if val_mape < self.model_performance.get("mape", float("inf")):
                    ...
                    logger.info(
                        f"Model updated to {self.model_version}, MAPE: {val_mape:.4f}"
                    )
    ...
    def prepare_training_data(self, data: pd.DataFrame) -> Tuple:
        ...
        try:
            ...
            for i in range(
                sequence_length, len(X) - 24
            ):  # Ensure we have future targets
                ...
                target_4h = (
                    data.iloc[i + 4]["close"] / data.iloc[i]["close"] - 1
                    if i + 4 < len(data)
                    else 0
                )
                target_24h = (
                    data.iloc[i + 24]["close"] / data.iloc[i]["close"] - 1
                    if i + 24 < len(data)
                    else 0
                )
            ...
            for i in range(0, len(X_tensor), batch_size):
                ...
                for horizon in ["1h", "4h", "24h"]:
                    batch_targets = torch.cat(
                        [
                            targets[j][horizon]
                            for j in range(i, min(i + batch_size, len(targets)))
                        ]
                    )
    ...
    async def evaluate_model(
        self, model: nn.Module, validation_data: pd.DataFrame
    ) -> float:
        ...
        try:
            ...
            feature_cols = [
                col for col in validation_data.columns if col in self.feature_columns
            ]
            X_val = self.scaler.transform(
                validation_data[feature_cols].fillna(0).values
            )

            with torch.no_grad():
                for i in range(24, len(X_val) - 1):
                    sequence = (
                        torch.FloatTensor(X_val[i - 24 : i]).unsqueeze(0).to(device)
                    )
                    ...
                    if "1h" in pred:
                        ...
                        actual = (
                            validation_data.iloc[i + 1]["close"]
                            / validation_data.iloc[i]["close"]
                            - 1
                        )
        ...
        while True:
            try:
                ...
                if current_mape > 0.05:  # 5% threshold
                    logger.warning(
                        f"Model performance degraded: MAPE {current_mape:.4f}"
                    )
    ...
    async def get_model_status(self) -> Dict:
        ...
        return {
            ...
            "prediction_count_24h": await self.redis_client.get("prediction_count")
            or 0,
        }
...
async def predict_single_endpoint(request: PredictionRequest):
    ...
    try:
        result = await tft_predictor.predict_single(
            request.ticker, request.features, request.horizons
        )
        return result
    except Exception as e:
