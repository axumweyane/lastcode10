...
class TradingEngine:
    ...
    async def load_portfolio_state(self):
        ...
        try:
            ...
            logger.info(
                f"Portfolio loaded: ${self.portfolio_value:,.2f}, {len(self.positions)} positions"
            )
    ...
    async def load_risk_parameters(self):
        ...
        try:
            ...
            await self.redis_client.set(
                "risk_params", json.dumps(self.risk_metrics), ex=3600
            )

        except Exception as e:
            logger.error(f"Risk parameters initialization failed: {e}")

    async def alpaca_api_call(
        ...
        try:
            ...
            async with self.alpaca_session.request(
                ...
                if response.status == 200:
                    ...
                    logger.error(
                        f"Alpaca API error: {response.status} - {await response.text()}"
                    )
                    return None
    ...
    async def place_order(self, request: OrderRequest) -> Dict:
        ...
        try:
            ...
            risk_check = await self.validate_risk(
                RiskCheckRequest(
    ...
    async def place_bracket_order(self, request: BracketOrderRequest) -> Dict:
        ...
        try:
            ...
            order_data = {
                ...
                "order_class": "bracket",
                "take_profit": {"limit_price": str(request.take_profit_price)},
                "stop_loss": {"stop_price": str(request.stop_loss_price)},
            }
            ...
            if response:
                ...
                if order_id in self.open_orders:
                    ...
                    await self.publish_order_update(
                        self.open_orders[order_id], "order_cancelled"
                    )
    ...
    async def validate_risk(self, request: RiskCheckRequest) -> Dict:
        ...
        try:
            ...
            if position_pct > self.risk_metrics["max_position_size"]:
                violations.append(
                    f"Position size {position_pct:.2%} exceeds limit {self.risk_metrics['max_position_size']:.2%}"
                )
            ...
            if request.side == OrderSide.BUY:
                ...
                if required_capital > self.buying_power:
                    violations.append(
                        f"Insufficient buying power: ${required_capital:,.2f} required, ${self.buying_power:,.2f} available"
                    )
            ...
            if sector:
                ...
                sector_pct = (
                    new_sector_value / self.portfolio_value
                    if self.portfolio_value > 0
                    else 1.0
                )
                sector_limit = self.risk_metrics["sector_limits"].get(
                    sector.lower(), 0.15
                )

                if sector_pct > sector_limit:
                    violations.append(
                        f"Sector allocation {sector_pct:.2%} exceeds {sector} limit {sector_limit:.2%}"
                    )

            # Daily loss limit check
            daily_pnl = sum(pos.day_pnl for pos in self.positions.values())
            daily_loss_pct = (
                abs(daily_pnl) / self.portfolio_value
                if daily_pnl < 0 and self.portfolio_value > 0
                else 0.0
            )
            ...
            # Cash reserve check
            if request.side == OrderSide.BUY:
                cash_after_trade = self.buying_power - (
                    request.quantity * request.price
                )
                cash_reserve_pct = (
                    cash_after_trade / self.portfolio_value
                    if self.portfolio_value > 0
                    else 0.0
                )
                ...
                if cash_reserve_pct < self.risk_metrics["min_cash_reserve"]:
                    violations.append(
                        f"Cash reserve would fall below minimum {self.risk_metrics['min_cash_reserve']:.2%}"
                    )
            ...
            return {
                "approved": approved,
                "reason": (
                    "Risk checks passed" if approved else "Risk violations detected"
                ),
    ...
    async def get_sector(self, ticker: str) -> Optional[str]:
        ...
        try:
            ...
            await self.redis_client.set(
                f"sector:{ticker}", sector, ex=86400
            )  # 24 hours
                    ...
                    for order_data in orders_data:
                        ...
                        if order_id in self.open_orders:
                            ...
                            if old_status != new_status:
                                ...
                                self.open_orders[order_id].filled_qty = int(
                                    order_data.get("filled_qty", 0)
                                )

                                if order_data.get("filled_avg_price"):
                                    self.open_orders[order_id].filled_price = float(
    ...
    async def update_portfolio_metrics(self):
        ...
        while True:
            try:
                ...
                # Cache metrics
                await self.redis_client.set(
                    "portfolio_metrics", json.dumps(metrics), ex=300
                )
        ...
        try:
            ...
            for sector in sector_allocations:
                sector_allocations[sector] = (
                    sector_allocations[sector] / total_value if total_value > 0 else 0
                )
