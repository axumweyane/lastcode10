...
def train_tft_model_from_database():
    ...
    async def load_market_data(
        conn: asyncpg.Connection, tickers: List[str], limit: int = 50000
    ) -> pd.DataFrame:
    ...
    def add_technical_indicators(df: pd.DataFrame) -> pd.DataFrame:
        ...
        for ticker in df["ticker"].unique():
            ...
            df.loc[mask, "volume_ratio"] = (
                ticker_data["volume"] / df.loc[mask, "volume_sma"]
            )

            # Price returns
            df.loc[mask, "returns"] = ticker_data["close"].pct_change()
            df.loc[mask, "returns_1h"] = ticker_data["close"].pct_change(
                60
            )  # 1-hour returns
    ...
    def add_time_features(df: pd.DataFrame) -> pd.DataFrame:
        ...
        ts_eastern = (
            ...
            else df["timestamp"].dt.tz_convert(eastern)
        )
    ...
    async def prepare_dataset(
        tickers: List[str] = ["AAPL"], limit: int = 50000
    ) -> pd.DataFrame:
    ...
    def create_tft_dataset(
        df: pd.DataFrame, max_encoder_length: int = 168, max_prediction_length: int = 24
    ...
    async def main():
        ...
        try:
            ...
            df = await prepare_dataset(
                tickers=tickers, limit=10000
            )  # Smaller dataset for testing

            # Create TFT datasets
            training_dataset, validation_dataset = create_tft_dataset(
                df, max_encoder_length=24, max_prediction_length=4
            )
            ...
            model = train_model(
                training_dataset, validation_dataset, max_epochs=5, batch_size=16
            )  # Reduced for testing

            # Evaluate model
            predictions = model.predict(
                validation_dataset.to_dataloader(train=False, batch_size=64)
            )
