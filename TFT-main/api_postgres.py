...
class PredictionRequest(BaseModel):
    ...
    include_attention: bool = Field(
        default=False, description="Include attention weights"
    )
...
async def get_symbol_info(
    symbol: str, db_loader: PostgresDataLoader = Depends(get_db_loader)
):
    ...
    try:
        ...
        fundamental_info = (
            fundamentals.iloc[0].to_dict() if not fundamentals.empty else {}
        )
